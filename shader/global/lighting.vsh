...
void init_generic() {
    ...
    LightmapCoords = clamp((gl_TextureMatrix[1] * gl_MultiTexCoord1).xy * 1.06667 - 0.0625, 0.0, 1.0);
    ...
    float hl = pow(clamp(1.0 - Dist / (heldBlockLightValue * 0.5), 0.0, 1.0), HANDHELD_FALLOFF_CURVE);
    #else
    ...
    float shadowTransition = smoothstep(0.85 - SHADOW_SOFTNESS, 0.96, LightmapCoords.y);
    #else
    float shadowTransition = smoothstep(0.85, 0.96, LightmapCoords.y);
    #endif

    SUN_AMBIENT += SUN_DIRECT * NdotL * shadowTransition;
    #endif
