// Optimized bloom - reduced texture samples and calculations

// Original blur3x3 function (5 samples)
...
vec2 adjust_vertex_position(float TileSize, float MaxSize, float TileOffset, vec2 glPos) {
    vec2 TileSizeClamped = vec2(aspectRatio, 1.0) * TileSize;
    float Scale = max(1.0, min_component(TileSizeClamped) / min_component(resolution * MaxSize));
    return (glPos * TileSizeClamped + TileOffset * vec2(aspectRatio, 1.0)) / Scale * resolutionInv;
}
