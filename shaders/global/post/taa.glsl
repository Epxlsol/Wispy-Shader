// Optimized TAA for Wynncraft - Reduced texture overhead
// Based on Filmic SMAA & Activision Research

// Reprojection: Finds where the current pixel was in the previous frame
...
vec3 clipAABB(vec3 prevColor, vec3 minColor, vec3 maxColor) {
    ...
    vec3 eClip = 0.5 * (maxColor - minColor) + 0.0001; // Added epsilon to prevent div by zero
...
vec3 texture_catmullrom_fast(sampler2D colorTex, vec2 texcoord) {
    ...
    float c = 0.5; // Adjusted sharpness constant
...
vec3 TAA(inout vec3 Color, vec3 CurrentPos, vec2 PrevCoordCenter, bool IsDH) {
    ...
    #if TAA_MODE == 3
    // Closest depth search (keeps edges sharp during movement)
