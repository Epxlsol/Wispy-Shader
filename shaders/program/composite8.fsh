...
void main() {
    ...
    // --- CAS Sharpening (Linked to Menu Slider) ---
    // Counteracts TAA/Bloom blur. Strength is now controlled by the slider.
    #ifdef SHARPENING
    ivec2 fragCoord = ivec2(gl_FragCoord.xy);
    vec3 center = Color.rgb;
    vec3 off = (
        texelFetch2D(colortex0, fragCoord + ivec2(1, 0), 0).rgb +
        texelFetch2D(colortex0, fragCoord + ivec2(-1, 0), 0).rgb +
        texelFetch2D(colortex0, fragCoord + ivec2(0, 1), 0).rgb +
        texelFetch2D(colortex0, fragCoord + ivec2(0, -1), 0).rgb
    );

    // SHARPENING usually ranges 0.0-1.0.
    // We multiply by 0.3 so 'Max' on the slider is a nice high-quality sharp.
    float sharpAmount = SHARPENING * 0.3;
    Color.rgb = clamp(center + (center * 4.0 - off) * sharpAmount, 0.0, 1.0);
    #endif
